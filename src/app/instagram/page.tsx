  ...
  return (
        <div className="space-y-4">
              <div className="bg-armadillo-card border border-armadillo-border rounded-xl p-5">
                      <div className="flex items-center justify-between mb-4">
                                <div className="flex items-center gap-2">
                                            ...
                                            <h2 className="text-sm font-medium text-armadillo-text">Trending Hashtags</h2>h2>
                                </div>div>
                      ...
                      <div key={tag.hashtag} className="bg-armadillo-bg rounded-lg p-3.5">
                                    <div className="flex items-center justify-between mb-1.5">
                                      ...
                                      {tag.trend === 'rising' && <span className="text-[10px] bg-success/20 text-success px-2 py-0.5 rounded-full flex items-center gap-0.5"><TrendingUp size={10} /> Rising</span>span>}
                                      {tag.trend === 'declining' && <span className="text-[10px] bg-danger/20 text-danger px-2 py-0.5 rounded-full flex items-center gap-0.5"><TrendingDown size={10} /> Declining</span>span>}
                                      {tag.trend === 'stable' && <span className="text-[10px] bg-armadillo-border text-armadillo-muted px-2 py-0.5 rounded-full">Stable</span>span>}
                                    </div>div>
                        ...
                        {tag.relatedHashtags.length > 0 && (
                                        <div className="flex flex-wrap gap-1">
                                          {tag.relatedHashtags.slice(0, 5).map(r => (
                                                              <span key={r} className="text-[10px] bg-armadillo-card text-armadillo-muted px-2 py-0.5 rounded">#{r}</span>span>
                                                            ))}
                                        </div>div>
                                    )}
                      </div>div>
          ...
          {displayPosts.length > 0 && (
                  <div className="bg-armadillo-card border border-armadillo-border rounded-xl p-5">
                            <h2 className="text-sm font-medium text-armadillo-text mb-4">Trending Posts by Hashtag</h2>h2>
